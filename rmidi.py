    ...
    class rmidi:
        ...
        def downloadmid(self, url, tmplocation, cookie, refreshtoken):
            ...
            if platform.system().lower() == "windows":
                ydl_opts["ffmpeg_location"] = filechecker.FileChecker().read_settings("settings.json")["ffmpeg_directory"]
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                try:
                    ...
                    if "ffprobe and ffmpeg" and "not found" in e.msg:
                        self.downloadprogress = f"[ERROR: {type(e).__name__}] An error has occured. Please set FFMPEG directory."
                        self.Log.writelog(f"[ERROR: {type(e).__name__}] An error has occured. Please set FFMPEG directory.", True)
